...
[`template_port`](https://github.com/missingno7/template_port) instead: start
at its `START_HERE.md` (the operational boot sequence) and its `docs/` (the
method — lifecycle, charter, pitfalls, the porting checklist). `template_port`
...
| [`glossary.md`](glossary.md) | Every project term (oracle, island, coastline, golden, heartbeat, …) in one table — shared vocabulary with `template_port`'s methodology docs. |
...
(`MIGRATION.md`) live in `template_port`, not here.
