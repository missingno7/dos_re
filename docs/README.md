...
`override_architecture.md` → `execution_planner.md` →
...
| [`override_architecture.md`](override_architecture.md) | **dos_re 3.0 override model:** reproducible baseline implementation providers plus one backend-neutral authored override registry; stable targets, faithful/enhancement/behavioral categories, verification policies, adapter boundary, and migration order. |
| [`execution_planner.md`](execution_planner.md) | **dos_re 3.0 launch model:** one universal `play.py`, orthogonal composition/execution/verification/build policies, mixed per-region bindings, detachment reports, a closed-world exporter, and atlas-independent coverage/evidence interfaces. The core contracts, standard profiles, pre-boot planner, report output, and real-mode player seam are landed; driver migration and export are phased next. |
