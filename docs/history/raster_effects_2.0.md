...
### Why this evidence is collected at the device boundary

Idioms vary per game and static recognition alone would be fragile. The
DOSMachine device model already observes ordered port I/O and virtual time, so
it can retain raw evidence without guessing its visual meaning. Interpreted and
generated implementations that select this device service can compare the same
journal. A detached renderer may instead consume a verified semantic projection;
it is not required to retain the DOS device model.
...
Any selected implementation whose palette discipline flows through the
DOSMachine's 3DAh waits and DAC writes can reuse this journal and classifier.
A port supplies only evidence-backed row mapping where the raw stream is
silent, plus whichever renderer consumes the result.
