...
| `frontend_timeline.py` | The **non-gameplay counterpart** to `tick_demo.py`: a per-present-frame timeline of the screens that run with NO game tick (intro / title / menu / attract / map / tally) — exactly where a VM-less front end drifts undetected (wrong screen ORDER, dropped fade, a screen before/after the wrong transition). `capture` (drive one present-frame at a time), `collapse` (run-length the timeline into a screen SEQUENCE), `diff_sequence` (screen order + per-run frame count, tolerant of sub-frame pacing) and `diff_pixels` (per-frame RGB digest, byte-exact). Cheap→strong: sequence gate always, pixel gate opt-in. Same oracle trick as the tick demo — capture the VM's per-frame sampled input and inject it into native, so the candidate makes the VM's choices with no synthetic keystrokes. Adapter classifies a frame from both sides to a common `(screen_id, rgb)`. |
