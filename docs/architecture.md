...
| `opl3.py` | The Yamaha OPL3 (YMF262) core: a literal pure-Python translation of Nuked-OPL3 v1.8 (LGPL-2.1-derived, see the file header and LICENSE), proven byte-identical to the upstream cffi reference build over golden scripts, full register-space fuzz and 80s of captured real game music before that reference was retired from this repo (`tests/test_opl3.py` keeps the golden hashes). Stdlib-only — no C compiler, identical under CPython/PyPy (real-time on CPython — ~1x busy/1.6x quiet — and ~30-60x under PyPy); `dos_re.audio_sink.load_opl3()` is the stable entry point. |
