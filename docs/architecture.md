...
| `input_demo.py` / `pm_input_demo.py` | Front-end input acquisition adapters used by the interactive players. They inject keyboard/mouse samples but are not proof-corpus artifacts and do not own checkpoints, bisection, or verification. New verification workflows wrap their event channels in `replay.py`. |
