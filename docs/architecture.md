...
| `replay.py` | **The dos_re 3.0 proof corpus:** deterministic events and stable points; execution-profile identities; profile-private complete continuation state; independent base-relative cached boundaries; raw-machine or canonical-semantic comparison projections; exact X→Y oracle/candidate verification; persistent divergence bisection; function visits. The same contract spans one hook through a detached native override profile. |
...
| Module | What it is |
|---|---|
| `player.py` | The universal project `play.py` frontend: resolves `development`/`verification`/`detached`/`release` plans before boot (`--plan-only` prints the report), then drives viewer, headless, replay, snapshot, input, pacing, and current hook compatibility. `GameFrontend` supplies coverage/catalog/service declarations and backend behavior. numpy/pygame imports stay lazy. |
