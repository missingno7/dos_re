...
| `input_demo.py` / `pm_input_demo.py` | Thin front-end adapters that normalize and apply real-mode or protected-mode input channels; the PM module also defines the adapter-supplied frame clock. They own no manifests, versions, snapshots, playback clocks, or persistence. Both players record directly into `ReplayArtifact`. |
| `replay.py` | **The dos_re 3.0 proof corpus:** deterministic events and stable points; execution-profile identities; profile-private complete continuation state; independent base-relative cached boundaries; raw-machine or canonical-semantic comparison projections; exact X→Y oracle/candidate verification; persistent divergence bisection; function visits; and optional oracle-owned observed-transfer evidence. The same contract spans one hook through a detached native override profile. |
| `identity.py` | Stable program, content-addressed image, function, execution-point, region, runtime-code slot/variant, and boundary identities shared by Recovery IR adapters, replay evidence, Atlas, catalogs, and plans. Identities never depend on generated Python names or a backend. |
| `atlas.py` | **The dos_re 3.0 Execution Atlas:** deterministic persistent source records and derived indexes; retained Recovery IR import without decoding; oracle replay-evidence ingestion; refused functions and unresolved frontiers; graph/replay queries; catalog/region joins; and conservative `CoverageSource` output for the planner. See [`execution_atlas.md`](execution_atlas.md). |
...
| Module | What it is |
|---|---|
| `lift/install.py` | The two install tiers: `install_vmless_graph` (install every emitted module; correctness judged by `replay.verify_interval`, divergences localized at stable cached points by `replay.bisect_divergence`) and `install_passing_lifts` (the hybrid tier — only oracle-passing modules). `resolve_links` binds liftlink's cross-module `LINKS` tables. |
