...
| `opl3.py` | The DORMANT bit-exact OPL3 core: a literal pure-Python translation of Nuked-OPL3 v1.8 (LGPL-2.1-derived; see LICENSE), proven byte-identical to the C reference (`tests/test_opl3.py` golden hashes). Kept verified as the reference for `opl3_fast` calibration and for possible future use; not the default playback backend anywhere. |
| `opl3_fast.py` | The CPython playback backend: a fast APPROXIMATE OPL3 (numpy block renderer, ~50x real-time on real game music, ~290x busy synthetic). Perceptually matched against the exact core — exact pitch math, calibrated ADSR, real stepped LFO patterns, the chip's actual rhythm phase-bit recipe, serial high-feedback recurrence — verified by `tests/test_opl3_fast.py` tolerance checks and an 80s real-music A/B. `dos_re.audio_sink.load_opl3()` picks per interpreter: compiled pynuked_opl3 when built, opl3_fast on CPython, exact `opl3.py` on PyPy. |
