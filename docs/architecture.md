...
| `demo_replay.py` | **dos_re 3.0 phase-1 hook-verification replay index:** stable points, cache identities, complete opaque continuation-state containers, persistent base-relative changed-page boundaries, exact oracle/candidate interval replay through a machine-specific driver seam, and recursion-safe function-visit metadata. Tick demos, frontend timelines, and playback-only artifacts are out of scope. |
