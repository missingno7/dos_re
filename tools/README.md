...
| `atlas.py` | `python tools/atlas.py build <atlas-dir> --ir recovery_ir.json --program GAME:VERSION --image-label main-exe --image-sha256 HASH --root CS:IP --product-profile development` | Build and query the persistent program map from retained IR and oracle replay evidence. Use `ingest-replay`, `show`, `callers`, `callees`, `coverage`, `best-replay`, `unresolved`, `path`, and `validate` on the same artifact. |
