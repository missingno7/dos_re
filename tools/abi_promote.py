"""abi_promote.py -- generate ABI-recovered modules from contract evidence
(dos_re.lift.emit_abi).

For every contract-promotable census entry with evidence-backed unobserved
outputs -- or every promotable entry with --all -- emit the
...
Usage:
    python dos_re/tools/abi_promote.py \
        ...
        --import-base game.recovered --abi-base game.recovered_abi \
        --out-dir game/recovered_abi [--all] [--apply]
"""
...
def _emit_cores(args, census, wanted) -> int:
    """Emit the de-stacked ABI core for every destackable leaf."""
    ...
    print("  kept mechanical (missing-capability work list):")
...
def main(argv=None) -> int:
    ...
    if args.apply and emitted:
        ...
        (out / "__init__.py").write_text(
            ...
            'ABI-recovered contract modules. Regenerate, '
            'do not hand-edit."""\n', encoding="utf-8")
