"""ABI-contract inference for ABI-recovered CPUless implementations.

The analysis removes the historical CPU calling convention from selected
public recovered contracts while the memory image may remain
...
#: Access width, in bytes, for the accepted ss-override forms.
...
def ss_globals_only(scan, floor=_NO_FLOOR):
    ...
    for i in scan.insts.values():
        ...
        #
        # So: an explicit table of the forms this analysis handles, and a REFUSAL
...
class ContractProposal:
    ...
    #: This function reaches globals through ss WHILE also using the
...
def infer_contracts(ir: dict, *, external: frozenset = frozenset(),
    ...
    """Infer ABI-contract evidence over a Recovery IR corpus.
    ...
    for key in sorted(ir["functions"]):
        ...
        # purely (ss_is_data_segment) or alongside real stack
    ...
    return {
        "_notice": "GENERATED by dos_re.lift.contracts.infer_contracts -- "
                   "the ABI-contract census. Regenerate, do not "
                   "hand-edit.",
        "summary": {
            "total": len(proposals),
