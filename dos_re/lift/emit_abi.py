"""ABI-recovered public-contract and proof-shadow emission over one core.
...
* the PUBLIC ABI-recovered entry (``abi_CCCC_IIII``, or the recovered name
  ...
  disappears when de-stacked core emission turns local push/pop into Python
  locals. Returns the caller-observed registers as a normal Python
...
Refusal-first: a proposal with refusals, or a shape this emitter does not
...
#: (needs the _ivec dispatch) and stays mechanical here; INT 20h
...
def emit_abi_module(key: str, proposal: dict, *, import_base: str,
    ...
    if "bp" in machine:
        # Framed stack arguments are unsupported: no framed functions in a
    ...
    stack_doc = ("\n    ``stack``: the (ss, sp) machine-stack view "
                 "(historical memory view; a de-stacked core removes it)."
                 if needs_stack else "")
    lines = [
        '"""AUTOGENERATED by dos_re.lift.emit_abi -- ABI-recovered contract',
        f'for {key} (dual entrypoints over the ONE mechanical',
...
# ---------------------------------------------------------------------------
# DE-STACKED algorithmic core for locally stack-contained functions
#
...
#: Virtual-stack slot deltas for the stack family this emitter virtualises.
...
def _vslot_delta(i) -> int | None:
    """Virtual-stack slot delta for a stack-family instruction this emitter
...
def check_composable(scan, *, callees=None, far_callees=None,
    ...
    Refusal-first; each reason is a capability name:
      * leaf-only: far/indirect/interrupt transfers need other capabilities;
    ...
    # A function with real stack traffic may STILL address globals
    ...
    for i in scan.insts.values():
        ...
        if i.op == 0x16 or (i.op == 0x8C and (i.reg & 3) == 2):
            ...
            # no ss; supporting this needs a semantic segment parameter.
        ...
        if not ss_data and not ss_globals:
            ...
            # SS-globals handling: only CONSTANT-offset accesses are proven
...
def check_destackable(scan, *, boundary_addrs=frozenset(),
    ...
    callees): every call is leaf-only-refused."""
...
def _promote_site(blk, start, i, promo, used) -> None:
    ...
    selected native-state accessor.
    ...
    if seg is None:
        raise Refusal(
            f"typed-memory-promotion: site {i.ip:#06x} has no memory operand")
    disp = i.imm if 0xA0 <= i.op <= 0xA3 else i.disp
    if disp != promo["disp"]:
        raise Refusal(
            f"typed-memory-promotion: site {i.ip:#06x} decodes disp {disp:#x}, schema "
    ...
    if promo.get("width", 1) != 1:
        raise Refusal(
            "typed-memory-promotion: only width-1 sites are supported")
    hits = 0
    ...
    if hits != 1:
        raise Refusal(
            f"typed-memory-promotion: site {i.ip:#06x} matched {hits} accesses of the "
...
def emit_abi_core(scan, proposal: dict, key: str, *,
    ...
    callers. Refuses unsupported shapes with a named missing capability; the
    refusal census is the resulting work list."""
    callees = callees or {}
    far_callees = far_callees or {}
    if proposal.get("refusals"):
        raise Refusal("contract-not-promotable")
    ...
    # A callee whose contract takes `ss` as a SEMANTIC data segment can only
    # be composed by a caller that itself carries ss --
    ...
    A(f'core for {key}. DO NOT hand-edit; regenerate.')
    A('')
    ...
    # NATIVE-STATE ACCESS SITES: accesses redirected to detached native state.
    ...
    # roles, so downstream consumers get pointer/segment structure explicitly
    ...
    if promotions and set(promotions) != used_promotions:
        ...
        raise Refusal(
            f"typed-memory-promotion: site(s) {[hex(m) for m in missed]} were in the "
...
def emit_integration_adapter(key: str, contract: CoreContract,
    ...
    exactly what the dropped-output perturbation replay validated: it
    XOR-perturbed the same
    registers and the replay stayed oracle-clean over 964 boundaries, so no
    observer exists to notice either substitution.

    The adapter deliberately provides a register-dict boundary for a selected
    mechanical caller while keeping the semantic core authoritative.
    """
    L: list[str] = []
    A = L.append
    ...
    A('    CPU-ABI boundary adapter for selected mechanical callers.')
    A('    Registers the core does not return are passed through: the census')
    A('    proved no caller observes them; dropped-output perturbation')
    A('    confirmed that evidence end to end on the replay."""')
...
def emit_shadow_loader(keys: list[str], *, abi_base: str,
    ...
    * RETRO-PATCHING covers the imports that already happened: mechanical
    ...
    cancel the perturbation (the self-edge composes conservatively anyway)."""
    stems = ", ".join(f'"{_stem(k)}"' for k in sorted(keys))
    return "\n".join([
        '"""AUTOGENERATED by dos_re.lift.emit_abi -- contract-proof shadow',
        'loader. DO NOT hand-edit; regenerate."""',
        '',
        'import importlib',
        'import sys',
        '',
